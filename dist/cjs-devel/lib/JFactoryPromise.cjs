...
const JFactoryTrace = require('./JFactoryTrace.cjs');
...
class JFactoryPromise extends Promise {
    ...
    constructor({ name, config, traceSource }, executor) {
        jFactoryBootstrap.jFactoryBootstrap_expected();
        ...
        {
            JFactoryExpect.JFactoryExpect("name", name)
                .type(String, Number)
                .matchReg(/^[^. ]+$/);
            JFactoryExpect.JFactoryExpect("config", config).typePlainObject();
            JFactoryExpect.JFactoryExpect("executor", executor).typeFunction();
        }
...

        {
            ...
            if (!jFactoryHelpers.helper_isNative(executor)) {
                Object.defineProperties(this, {
                    $dev_source: {
                        value: executor
                    }
            ...
            JFactoryTrace.jFactoryTrace.attachTrace(this, traceSource);
        }
    ...
    then(onFulfilled, onRejected, forceType) {
        ...
        let type = forceType || (
            jFactoryHelpers.helper_isNative(onFulfilled) && !onFulfilled.name.startsWith("bound ") &&
            jFactoryHelpers.helper_isNative(onRejected) && !onRejected.name.startsWith("bound ")
    ...
    $chainComplete(reason = "$chainComplete()") {
        if (this.$chain.isPending) {
            throw new JFactoryError.JFACTORY_ERR_INVALID_CALL({
                target: this,
...
class JFactoryPromiseChain {
    ...
    constructor(chainRoot, chainId, chainName, chainConfig) {
        Object.defineProperties(this, {
            ...
            data: { value: {} },
            __deferred__: { value: jFactoryHelpers.helper_deferred() }
        });
    }
    ...
    complete(reason = "chain.complete()", abort) {
        ...
        if (!chainRoot.$isExpired) {
            /*let errorExpired = */chainRoot.$chain.errorExpired = new JFactoryError.JFACTORY_ERR_PROMISE_EXPIRED({
                target: chainRoot,
                reason
            });
...
const CONFIG = /*#__PURE__*/jFactoryConfig.jFactoryCfg("JFactoryPromise", {
