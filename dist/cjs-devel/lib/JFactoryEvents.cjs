...
const JFactoryFunction = require('./JFactoryFunction.cjs');
...
require('lodash/defaultsDeep.js');
...
class JFactoryEvents {
    ...
    on({ events, handler, target, selector }) {
        {
            ...
            JFactoryExpect.JFactoryExpect("JFactoryEvents.on({handler})", handler).typeFunction();
            target && JFactoryExpect.JFactoryExpect("JFactoryEvents.on({target})", target).type(String, jQuery, HTMLElement);
            selector && JFactoryExpect.JFactoryExpect("JFactoryEvents.on({selector})", target).typeString();
        }
    ...
    off({ events, handler, target, selector }) {
        {
            if (events !== undefined) { // off() is valid
                JFactoryEvents.validateSelector(events);
                handler && JFactoryExpect.JFactoryExpect("JFactoryEvents.off({handler})", handler).typeFunction();
                target && JFactoryExpect.JFactoryExpect("JFactoryEvents.off({target})", target).type(String, jQuery, HTMLElement);
                selector && JFactoryExpect.JFactoryExpect("JFactoryEvents.off({selector})", target).typeString();
            }
        }
    ...
    triggerParallel({ events, data, target }) {
        {
            JFactoryEvents.validateSelector(events);
            target
                && JFactoryExpect.JFactoryExpect("JFactoryEvents.triggerParallel({target})", target).type(String, jQuery, HTMLElement);
        }
    ...
    triggerSeries({ events, data, target }) {
        {
            JFactoryEvents.validateSelector(events);
            target && JFactoryExpect.JFactoryExpect("JFactoryEvents.triggerSeries({target})", target).type(String, jQuery, HTMLElement);
        }
...
class JFactoryEventsManager extends JFactoryEvents {
    constructor(parent) {
        ...
        Object.defineProperties(this, {
            parent: { value: parent },
            affiliateRules: { value: new JFactoryFunction.JFactoryFunctionComposer().compose() }
        });
...

{
    JFactoryEvents.validateSelector = function(selector) {
        {
            JFactoryExpect.JFactoryExpect("JFactoryEvents.validateSelector(selector)", selector)
                .notUndefined()
                .validSpaces();
        }
    ...
    JFactoryEvents.validateEvent = function(event) {
        {
            JFactoryExpect.JFactoryExpect("JFactoryEvents.validateEvent(event)", event)
                .notUndefined()
                .notEmptyString()
                .validSpaces();
        }
    ...
    JFactoryEvents.validateNamespace = function(namespace) { // ex: ns.ns2.ns3
        {
            JFactoryExpect.JFactoryExpect("JFactoryEvents.validateNamespace(namespace)", namespace)
                .notUndefined()
                .notEmptyString()
                .validSpaces();
        }
